...
This is a file with exercises for the ML 2020 course. The exercises are divided in two parts. This is part 1: Principal components analysis. 
...
- Advanced exercises, you can safely skip it for now
...
```
There are two datasets in your environment: `rna` (transcripts) and `metab` (metabolites).
...
- Why is it better to derive singular vectors fro $X$ than eigenvectors from $X^\top X$, where $X$ is the `rna` dataset?
